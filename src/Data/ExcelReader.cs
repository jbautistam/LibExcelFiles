...
namespace Bau.Libraries.LibExcelFiles.Data
{
	...
	public class ExcelReader : IDataReader
	{
		...
		/// </summary>
		public void Open(System.IO.Stream stream, int sheetIndex)
		{
			...
			RowModel rowData = _manager.GetRow(2);
				...
				for (int index = 0; index < rowData.Cells.Count; index++)
				{
					string name = $"Column{index.ToString()}";
						...
						if (WithHeader)
							name = rowHeader.Cells[index].Value?.ToString();
						// Añadimos la columna
		...
		{
			return GetDataValue<DateTime>(i);
		}
		...
		{
			return index >= _recordsValues.Count || _recordsValues[index] == null || _recordsValues[index] is DBNull;
		}
		...
		///		Columnas
		/// </summary>
		public List<(string name, Type type)> Columns { get; } = new List<(string name, Type type)>();
